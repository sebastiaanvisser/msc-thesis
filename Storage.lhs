...
|Binary| instance for it, which can be derived because the type is just a
|newtype| over an |Integer|.

> newtype Pointer (f :: * -> *) a = Ptr { unPtr :: Integer }                               
>   deriving Binary
...
> read :: Binary (f a) => Pointer f a -> HeapR (f a)
...
> allocate :: Integer -> HeapA (Pointer f a)
...
> update :: Binary (f a) => f a -> Pointer f a -> HeapW ()
...
> write :: Binary (f a) => f a -> HeapW (Pointer f a)
