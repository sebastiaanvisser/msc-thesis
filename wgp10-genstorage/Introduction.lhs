...
build our own system that matches the structure of algebraic datatypes.
...
on disk called \texttt{squares.db} and uses it to store a mapping from
numbers to their squares. The second program, that can run independently from
the first program, opens up the same database and starts a loop that
interacts with the user. In every iteration, the program expects a number
and tries to look up the square of that number in the database. When a square
is found this will be reported back to the user, when no square is found, it is
