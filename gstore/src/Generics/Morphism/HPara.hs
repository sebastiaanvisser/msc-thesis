...
data AlgA (a :: ((* -> *) -> * -> *) -> (* -> *) -> * -> *)
          ...
          (r :: *)
  where
  Psi  :: (f (HFixA a h ix, r) -> r)  -> AlgA a h r
...
-- _para :: (Traversable f, Lazy m, AnnQ a f m) => (x -> m r) -> (r -> x) -> AlgA a f x -> HFixA a f -> m r
...
_para :: ({-Traversable h,-} Lazy m, HAnnQ a h ix m) => AlgA a h r -> HFixA a h ix -> m r
_para (Psi psi) = undefined -- return . psi <=< mapM (grp (fmap id . lazy . _para (Psi psi))) <=< runQuery
--   where grp f c = fmap ((,) c) (f c)
