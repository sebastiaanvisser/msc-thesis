...
\subsection{Clean}
  ...
  data structures to a persistent data storage on disk.  Using something
  similar as our storage heap -- they call this \emph{Chunks} -- they are able
  to persist individual parts of the data structures on disk without reading or
  writing the entire collection.
  }
  ...
  without touching the entire collection. But, for every change in the
  structure of the collection the chunk containing the data structure itself
  --- they call this the \emph{Root chunk} --- has to be read in and written
  back as a whole.
  }

  \fancy{
  Because of the different method of slicing the data they do not pose the same
  problems in lifting the pure structures to a persistent variant. Most
  container data type definitions already are polymorphic in the element values
  they store. This significantly simplifies their implementation while making
  the system less flexible.
  }

  \fancy{
  To improve type safety of their framework these Clean developers include a
  type hash inside their chunks. When reading data back in this type will be
  checked for validity. This is a very interesting technique that might also be
...
\subsection{Binary serialization}
  ...
  library that enables generic serialization of values of arbitrary data types
  to binary streams. The library uses type classes to be polymorphic in the
  values to be serialized/deserialized. Out of the box the library forces users
  to write the type class instances for their data types by hand, but luckily
  ...
  generically derive instances as well. The advantage is that this can be used
  as a library and does not require external inclusion into your project.  It
  might be interesting to note that using the above mentioned generic
  programming libraries -- of which there are quite a few \cite{compgen} -- it
  should be fairly easy to skip the Data.Binary library entirely and write the
  functions from scratch, as described in \cite{printparse} and \cite{clean}.
  }
...
\subsection{Algebras}
  ...
  and Barbed Wire} Meijer, Fokkinga and Paterson\cite{bananas} show how certain
  ...
  algorithm, they only need a few basic function to handle the actual
  recursion. Morphisms like catamorphisms and paramorphisms can be used to
  destruct a data type into a new value and can be seen as consumer functions.
  Anamorphisms can be used to create values of a data type and can be used to
  create producer functions. By combining or restricting these morphisms most
  if not all\cite{paramorphisms} algorithms working on functional data
  structures can be written without explicit recursion.
  }
...
\subsection{Attribute Grammars}
  ...
  \fancy{
  Attribute grammars can be seen as a way of writing algebras for
  catamorphisms. The attribute grammar system for Haskell\cite{ag} allows users
  to write operations over data structures in an aspect oriented way without
  explicitly coding out the recursion.
  }
...
\subsection{Iteratee based IO}
  ...
  \fancy{
  The ideas from his work can be used to avoid the same pitfall in this
  project. We should make sure that processing values originating from our
  storage heap in pure functions does not cause any effects.
  }
...
\subsection{Software Transactional Memory}
  ...
  without the intervening of other threads.
  }
