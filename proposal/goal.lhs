...
manage all low level IO operations. We at least need one sufficiently efficient
heap implementation with the interface described in the previous sections.
...
based functions to persistent operations. This would most certainly imply using
some generic programming library to derive monadic variants of catamorphisms,
anamorphisms and paramorphims and accompanying algebras and coalgebras.
...
algorithms. This part of the project is somewhat open and needs some additional
research.
...
structure and good efficiency is the finger tree\cite{fingertree}. Finger trees
are the based of the Haskell |Data.Sequence| data structure.
...
or overwrite the original value. These values might be shared among other parts
of the structure.  To make sure that the storage heap does not grow with every
modification we are in need of a garbage collector to clean up unused blocks of
data. 
...
collector model that can be implemented is a reference counting garbage
collector. Reference counting garbage collectors are infamous because of their
inability to detect and cleanup cycles. 

By initially only allowing the persistence of non-cyclic structures with
possible sharing we can show that the framework does have support for sharing.
In a later time it might be desirable to make a less conservative garbage
collector that can also cleanup cyclic structures.

\subsection{Transactional cache}

To allow safe and concurrent access to the underlying storage layer we need to
implement or use some form of transactional cache. This is a single entry point
that runs operations on the persistent data structure using an in-memory
...
To illustrate that the use of the framework and to be forced to think about the
exposed API at least one demo application should be build. This program should
illustrate the use of the underlying framework by storing a single data
structure containing a reasonable large collection of records. All common
operations should be included in the demo, like creating a storage from an
existing list of records, inserting new record, querying records by some index,
removing records and counting the number of records.
