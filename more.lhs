...
\fancy{The previous sections describe, at a very basic level, what components
are essential to come up with a system for data persistency in Haskell that
fulfils our demands. We need a layer for flexible binary data storage, a layer
...
\begin{itemize}
  ...
  \item
  \fancy{What kind of storage heap can be used to efficiently allocate blocks
  of data on demand and how can be learn from existing in-memory heaps? Do we
  need automatic garbage collection of unused blocks, or do we exactly know
  when to free blocks by analysing the behaviour of our container algorithms?}

  \item
  \fancy{Can we improve overall performance by caching or fusing several read
  or write actions in-memory into one single action that shares common disk
  accesses? Can this fusion be expressed implicitly, without adapting our
  interfaces.}

  \item
  \fancy{The system described in this document allows for an aspect oriented
  way of programming by generically annotating data structure and algorithms
...
> type MyDB a b = Persistent (Caching (FTree a b))
  ...
  \item
  \fancy{What happens when we try to store infinite data structures that
  ...
  \item
  \fancy{What container data structures can we write to efficiently store and
...
\fancy{The goal of this project is to come up with a working system for data
